...
def predict_similarity(question_left, question_right):
	...
	# load final model
	model = model_from_json(open(os.path.join(cwd,  "models/lowercase_model_80_42")).read())
	model.load_weights(os.path.join(cwd, "models/lowercase_model_80_42") + '.weights')
	...
	question_1 = unicode(question_left.lower())
	question_2 = unicode(question_right.lower())
